...
# AWS Sandbox Provisioner Prerequisite script
# Make sure you have set all the necessary environment variables and configurations before running this script.

set -e
export policy_name="SandboxProvisionerPolicy"        # name of the policy which will be used by the role responsible for orchestrating the accounts under sandbpx provisioner
export policy_file="sandbox_provisioner_policy.json" # name of the policy document for above policy
...
export RUNNER_INSTANCE_TYPE="t2.micro"
...
export CREATE_RUNNER="true" # Do not change
self_hosted_runner_prerequisites_check() {
    ...
    for existing_cidr in $existing_vpcs; do
        if [ "$existing_cidr" == "$SELF_HOSTED_RUNNER_VPC_CIDR" ]; then
            echo -e "${RED}\nError: CIDR block $SELF_HOSTED_RUNNER_VPC_CIDR conflicts with an existing VPC.${NC}"
            echo -e "$GREEN"
            read -rp "Continue if it is a re-run. Continue? (y/n): " continue
            echo -e "$NC"
            if [[ "$continue" =~ ^[Yy]$ ]]; then
                echo "continuing..."
            else
...
check_cut() {
    if ! command -v cut &>/dev/null; then
        echo "Cut is not installed. Please install Cut to use this script."
        exit 1
    fi
}
...
create_management_role() {
    ...
    if ! aws iam create-role --role-name "$SANDBOX_MANAGEMENT_ROLE_NAME" --assume-role-policy-document "$MGMT_ROLE_POLICY_JSON" &>/dev/null; then
        print_message "Failed to create the Management IAM Role: '$SANDBOX_MANAGEMENT_ROLE_NAME'" "$RED"
        exit 1
    fi
...
create_lambda_role() {
    ...
    if ! aws iam create-role --role-name "$lambda_role_name" --assume-role-policy-document "$LAMBDA_ROLE_POLICY_JSON" &>/dev/null; then
        print_message "Failed to create the Lambda IAM Role: '$lambda_role_name'" "$RED"
        exit 1
    fi
...
main() {
    ...
    # This is the main function responsible for provisioning the required resources and updating GitHub workflow files.
    # It performs tasks like checking prerequisites, creating IAM policies, roles, and secrets, updating workflow files,
    # and creating Self-Hosted GitHub runners if required.
    #
    # Make sure you have set all the necessary environment variables and configurations before running this script.
    ...
    if [[ $REPO_URL == https://* ]]; then
        export REPO_OWNER=$(echo $REPO_URL  | awk -F '.com/' '{print $2}' | cut -d '/' -f1)
    elif [[ $REPO_URL == git@* ]]; then
        export REPO_OWNER=$(echo $REPO_URL | awk -F ':' '{print $2}' | cut -d '/' -f1)
    fi
    ...
    echo -e "${RED}----------------------------------------------------${NC}"
    print_message "Below resources will be created and workflow files will be updated with the variables." "$GREEN"
    sleep 1
    echo -e "
...
"
    ...
    echo -e "Default region    : ${YELLOW}$AWS_DEFAULT_REGION${NC}"
    echo -e "GitHub Repo Owner : ${YELLOW}$REPO_OWNER${NC}"
    echo -e "GitHub Repo Name  : ${YELLOW}$REPO_NAME${NC}"
    sleep 1

    echo -e "\nMake sure you are authenticated to the management account and the details above are as expected."
    echo -e "$GREEN"
    read -rp "Do you want to continue? (y/n): " continue
    echo -e "$NC"

    # Prompt user to enter GitHub token (github_token)
    read -rp "Enter GitHub token [ Probably Bot User ] with access to the repository '$REPO_NAME' and workflows.] : " github_token
    ...
    if [[ ! $token_scopes == *workflow* ]]; then
        echo -e "\n${RED}IMPORTANT${NC} GitHub Token does not have the ${YELLOW}'workflow'${NC} scope."
    ...
    export GITHUB_USERNAME=$(echo "$USER_RESPONSE" | jq -r .login)
    ...
    if [ -z "$PARENT_OU_ID" ]; then
        ...
        echo -e "\nUsing ${YELLOW}${PARENT_OU_ID} ${NC} as parent to deploy the organizational unit [OUs] for sandbox provisioner."
        echo -e "$GREEN"
        read -rp "Do you want to continue? (y/n): " continue
        echo -e "$NC"
    ...
    if [ "$SSO_ENABLED" = "true" ]; then
      ...
      if [ -n "$SSO_INSTANCE_INFO" ]; then
        ...
        if [[ "$continue" =~ ^[Yy]$ ]]; then
            ...
            SANDBOX_USER_PERMISSION_SET_ARN=$(aws sso-admin create-permission-set \
            ...
            aws sso-admin attach-managed-policy-to-permission-set \
              --instance-arn $SSO_INSTANCE_ARN \
              --permission-set-arn "$SANDBOX_USER_PERMISSION_SET_ARN" \
              --managed-policy-arn "$MANAGED_POLICY_ARN_FOR_SANDBOX_USERS"

            sleep 1

            echo -e "Sandbox user Permission set ARN : ${YELLOW}$SANDBOX_USER_PERMISSION_SET_ARN${NC}"
        ...
        if [ -f "../.github/workflows/aws-provision.yml" ]; then
          find ../.github/workflows -type f -iname "*.yml" -exec bash -c "m4 -D REPLACE_SSO_ENABLED_FLAG_HERE=${SSO_ENABLED} -D INSTANCE_ARN_PLACEHOLDER=$SSO_INSTANCE_ARN -D IDENTITY_STORE_ID_PLACEHOLDER=$SSO_IDENTITY_STORE_ID -D REPLACE_SANDBOX_USER_PERMISSION_SET_ARN_HERE=$SANDBOX_USER_PERMISSION_SET_ARN {} > {}.m4  && cat {}.m4 > {} && rm {}.m4" \;
...
EOL
    ...
    # Create Runner role
    if [[ $CREATE_RUNNER == "true" ]]; then
      if aws iam get-role --role-name "$GITHUB_RUNNER_ROLE_NAME" &>/dev/null; then
        echo "$GITHUB_RUNNER_ROLE_NAME already exists. Using the same"
      else
        GITHUB_RUNNER_ROLE_OUTPUT=$(aws iam create-role --role-name "$GITHUB_RUNNER_ROLE_NAME" --assume-role-policy-document '{"Version": "2012-10-17","Statement": [{"Effect": "Allow","Principal": {"Service": "ec2.amazonaws.com"},"Action": "sts:AssumeRole"}]}' --description "Role for GitHub Runner" --output json)
      fi
    fi
    ...
    echo -e "${RED}----------------------------------------------------${NC}"
    echo -e "Setup Done, Pushing workflow file changes to remote repo"
    echo -e "${RED}----------------------------------------------------${NC}"
    ...
    echo -e "${RED}----------------------------------------------------${NC}"
    echo -e "        Done... Happy Sandboxing !!!"
    echo -e "Merge the recently pushed branch to main/default to start using 🚀🚀🚀"
    echo -e "${RED}----------------------------------------------------${NC}"
